...
;;;;;;;::::;:;::;;;::::;;;;::::;::;::;;:;;::;;;;::::;;:;;::;;;:;::;:;;;::;;:;;::;;::;::;;;:;::;;:;
;;;   		      	 
;;; zcavdar14@ku.edu.tr     Tue Nov  1 11:31:04 2016
;;;   		      	 
;;; Before you start:   		      	 
;;;   		      	 
...
;     * code files to read: curve_utils.scm, curves.scm, curves_setup.scm,
;       points_segs.scm   		      	 
;     * code file simply to load: drawing.scm
...
;;; Graphics   		      	 
;   Mit-Scheme and Racket use different primitives to handle graphics
;   In this project, drawing and curves_setup files are related to graphics
;   Load the platform specific versions of that files
;   If you are using Mit-Scheme, load drawing.scm and curves_setup.scm
;   If you are using Racket, load drawing_racket.scm and curves_setup_racket.scm
...
; If you are using mit-scheme load following files
; (load "drawing.scm")   		      	 
; (load "curves_setup.scm")   		      	 
   		      	 
; If you are using racket load racket version of above files
...
(define (segment-length seg)   		      	 
  (sqrt
   (+ (square (- (x-of (start-seg seg)) (x-of (end-seg seg))))
...
;(define seg2 (make-seg (make-point 0 0)
;		      (make-point 6 8))
   		      	 
(segment-length seg)   		      	 
; will yield the value 5.830951894845301
;(segment-length seg2)
; will yield the value 10
...
(define (perimeter lst)
  (define (running-sum sum poly)
    (if (null? poly)
        sum
        (running-sum (+ sum (segment-length (car poly))) (cdr poly))))
  (running-sum 0 lst)
)   		      	 
...
(define (copy-first-to-last lst)
  (append lst (list (car lst))))

(define (create-polygon ptlist)
  (define (appender input)
    (if (null? (cdr input))
        '()
        (cons (make-seg (car input) (cadr input)) (appender (cdr input))))
    )
  (appender (copy-first-to-last ptlist)))   		      	 
...
(define (add-two-points p1 p2)   		      	 
  (make-point (+ (x-of p1) (x-of p2)) (+ (y-of p1) (y-of p2)))
)   		      	 
...
(define (add-all-points lst)
  (define (summer res input)
    (if (null? input)
        res
        (summer (add-two-points res (car input)) (cdr input))
        ))
  (summer (make-point 0 0) lst)
)   		      	 
...
(define (centroid lst)
  (let ((all-sum (add-all-points lst)))
    (make-point (/ (car all-sum) (length lst)) (/ (cadr all-sum) (length lst)))
    )
)   		      	 
...
(define (centroid-alt lst)
  ...
  (define (sum-all-x res input)
    (if (null? input)
        res
        (sum-all-x (+ res (x-of (car input))) (cdr input))))
  
   (define (sum-all-y res input)
    (if (null? input)
        res
        (sum-all-y (+ res (y-of (car input))) (cdr input))))
  (make-point (/ (sum-all-x 0 lst) (length lst)) (/ (sum-all-y 0 lst) (length lst))) 
)   		      	 
...
; MY ANSWER:
; Because square is a procedure which expects Number and returns Number.
; When (square cos) is tried to be evaluated by Scheme, cos is a procedure (Number -> Number) not a number itself.
; square cannot multiply a procedure with itself and gives error, because it expects Number.   		      	 
...
; MY ANSWER:
; (abbreviating Number-> Number as F)
; the result is
; Binary Operation -> (F,F -> F)   		      	 
...
; Because square-a-procedure expects a N->N procedure as an input. But deriv is prodecure
; with type (N->N)->(N->N) and it does not match.
...
(define deriv-squared (square-an-operator-on-proc deriv))
  ...
  ; MY ANSWER
  ; It works, because op-on-proc expects a function and returns a function (N->N). deriv can fit
  ; as op-on-proc and could be evaluated with function f.
...
;; MY ANSWER
;; (((thrice thrice) f) 0) will call f 27 times because it is equal to:
;; (thrice thrice) = (thrice of thrice of thrice)
;; thrice of f = fofof
;; thrice of fofof = fofofofofofofofof
;; thrice of fofofofofofofofof = fofofofofofofofofofofofofofofofofofofofofofofofofofof = 27 times calling f
;; That's why ((repeated f 27) 0) will return the same result.
...
;    1. What is the type of unit-line-at?
       ...
       ; N -> (N->Point)
       ; where Point is a list of 2 numbers
...
(define (diagonal-line pt len)
  (lambda (t)
    (make-point
     (+ (x-of pt) (/ (* len t) (sqrt 2)))
     (+ (y-of pt) (/ (* len t) (sqrt 2)))
     )
  )
)   		      	 
...
(define (upside-down curve)
  (lambda (t)
    (let ((ct (curve t)))
      (let ((c0 (curve 0)))
        (make-point (x-of ct) (- (* 2 (y-of c0)) (y-of ct)))))))
...
(define (connect-ends curve1 curve2)
  (lambda (t)
     (let* ((dif-x (- (x-of (curve1 1)) (x-of (curve2 0))))
            (dif-y (- (y-of (curve1 1)) (y-of (curve2 0))))
            (fitted-curve2 ((translate dif-x dif-y) curve2)))
       (if (< t (/ 1 2))   		      	 
              (curve1 (* 2 t))   		      	 
              (fitted-curve2 (- (* 2 t) 1)))
          )))
...
; MY ANSWER:
; There is a little graphics difference between unit-circle and alternative-unit-circle due to the
; equations they have. for t = 0.6, unit-circle draws the point ((sin 2*pi*0.6), (cos 2*pi*0.6)) while
; alternative-unit-circle calculates square of t. for t= 0.6, alternative-unit-circle draws the the point ((sin 2*pi*0.36), (cos 2*pi*0.36))
; except of the starting and ending points, the curves draw different points but the graphs look like similar.
; the difference can be easily observed when we use draw-points-squeezed-to-window
; In this drawing method, while unit-circle has equal distances between consecutive points,
; alternative-unit-circle will have ever-growing distances due to the exponential square of t.
; Difference between using draw-points-on and draw-connected is while the former one only display points corresponds to t,
; draw-connected connects consecutive points (t, t+1).
; squeezed-to-window procedures scales the curve such that it fits into the window.
...
(define arc
  (put-in-standard-position (lambda (t)
    (unit-circle (/ t 2)))
  ))   		      	 
...
(define (param-gosperize theta)
  (lambda (curve)   		      	 
    (put-in-standard-position   		      	 
     (connect-ends
      ((rotate-around-origin theta) curve) ((rotate-around-origin (- theta)) curve))
      )))   		      	 
...
;; MY ANSWER:
;((draw-points-squeezed-to-window g1 500) (param-gosper 4 (lambda (n) (/ pi (+ n 2)))))
;((draw-points-squeezed-to-window g2 500) (param-gosper 4 (lambda (n) (/ pi (expt 1.3 n)))))
;((draw-points-squeezed-to-window g3 500) (param-gosper 4 (lambda (n) (/ pi (expt 1.6 n)))))
...
(define (kochize level)
  (param-gosper (* 2 level) (lambda (x)
                    (if (even? x)
                        (/ -pi 6)
                        (/ pi 6)))))

(define (koch-curve level)   		      	 
     ((scale 0.5)((translate 0.5 1) (put-in-standard-position (kochize level)))))

;((draw-points-on g1 1000) (koch-curve 0))
;((draw-points-on g2 1000) (koch-curve 1))
;((draw-points-on g3 1000) (koch-curve 2))
;((draw-points-on g1 1000) (koch-curve 3))
...
(define degree-120 (/ (* 2 -pi) 3))
(define degree-240 (/ (* 4 -pi) 3))
(define degree-60  (/ (* 1 -pi) 3))
(define degree+60  (/ (* 1 pi) 3))
(define degree-45  (/ -pi 4))
(define degree+45  (/ pi 4))


(define triangle
  (connect-ends ((rotate-around-origin degree-60) unit-line) ((rotate-around-origin degree+60) unit-line)))

(define sfac 0.61)

(define (special-zafer unitcurve level)
   (if (= level 0)   		      	 
       unitcurve
       (connect-ends (connect-ends
                            (special-zafer ((scale sfac) unitcurve) (- level 1))
                            unitcurve)
                     (special-zafer ((scale sfac) unitcurve) (- level 1)))
       ))


(define (zafer-curve level)   		      	 
     ((translate 0 0.5)(put-in-standard-position (special-zafer triangle level))))

((draw-points-on g1 2500) (zafer-curve 3))

(define (special-zafer-curve curve level)
  (if (= level 0)
      curve
      (connect-ends
       (special-zafer-curve ((rotate-around-origin degree-45) curve) (- level 1))
       (special-zafer-curve ((rotate-around-origin degree+45) curve) (- level 1)))))
      
((draw-connected g2 3000) ((scale 0.6)((translate -0.2 0.5)(special-zafer-curve (zafer-curve 2) 1))))  
